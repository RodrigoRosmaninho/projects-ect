...
// ############################################
//  Fazer GET REQUEST a '/list?type=songs' para obter um objeto JSON com a lista de todos os excertos existentes.
...
function init_list() {
        ...
        if (song_list_request.readyState == 4 && song_list_request.status == 200) {
            ...
            <tr class="song page` + page + `">
                ...
                <td style="padding-top: 25px;">
                    <button style="margin-bottom: 15px;" type="button" class="btn btn-info" onclick="location.href='/create?id=` + song_data.id + `';">🎶  Remix</button>
                    <button style="margin-bottom: 15px;" type="button" class="btn btn-success" onclick="location.href='/getImage/?id=` + song_data.id + `';">🎵  Pauta</button>
    ...
    if (order != null) {
        song_list_request.open("GET", "list/?type=songs&order=" + order + "&asc_desc=" + a_d, true);
    ...
    else song_list_request.open("GET", "list/?type=songs&asc_desc=" + a_d, true);
...
function upvote(id) {
    ...
    if (!exists) {
        ...
        upvote_request.open("POST", "vote/?id=" + id + "&user=" + user + "&points=1&type=songs", true); // true for asynchronous 
...
function unvote(id, mode) {
    ...
    unvote_request.open("POST", "vote/?id=" + id + "&user=" + user + "&points=0&type=songs", true); // true for asynchronous 
...
function getAdmins() {
    ...
    admin_request.open("GET", "admins", true);
...
function changeSort(sort) {
    ...
    if (sort != undefined) window.location.replace("songs?order=" + sort + "&asc_desc=" + asc_desc);
    else window.location.replace("songs?asc_desc=" + asc_desc);
}
